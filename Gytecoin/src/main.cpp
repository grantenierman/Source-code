...
#if defined(NDEBUG)
# error "Gytecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x46699b0a17ce969825fb747b32735bb0a2f985e88d995bda7df57343366bc059");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gytecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gytecoin Signed Message:\n";
...
{
    // Gytecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Gytecoin: 1 days
static const int64 nTargetSpacing = 60; // Gytecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gytecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gytecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Gytecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7aed1c4fefed12dd8ca13485f40f3cf94203467ae4cb4523403a5c1a20b9d196");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Joan Rivers dead Comedian dies in New York, aged 81 04092014 The Independent 05092014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1409934486;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 709979;

        if (fTestNet)
        {
            block.nTime    = 1409934460;
            block.nNonce   = 381234;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xf104e9a86db416b79ed603829c4022d1c3cd66c3261239d25f1758286e085c6c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Gytecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GytecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GytecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GytecoinMiner(CWallet *pwallet)
{
    printf("GytecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gytecoin-miner");
    ...
    try { loop {
        ...
        printf("Running GytecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GytecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GytecoinMiner, pwallet));
}
